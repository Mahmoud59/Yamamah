                            ...
                            <div class="col-sm-12">
                                <div class="card">
                                    ...
                                    <div class="card-block card-dashboard">
                                        ...
                                        <div class="form-group col-sm-6 {!! formError($errors,'company_id',true) !!}">
                                            <div class="controls">
                                                {!! Form::label('company_id', __('company_id').':') !!}
                                                <input type="number" step="1" min="0" class="form-control" name="company_id" value="@if(!empty($result->payment_sdk_data->company_id)){{$result->payment_sdk_data->company_id}}@endif" placeholder="{{__('0_if_none')}}"/>
                                            </div>
                                            {!! formError($errors,'company_id') !!}
                                        </div>
                                        <div class="form-group col-sm-6 {!! formError($errors,'company_cat_id',true) !!}">
                                            <div class="controls">
                                                {!! Form::label('company_cat_id', __('company_cat_id').':') !!}
                                                <input type="number" step="1" min="0" class="form-control" name="company_cat_id" value="@if(!empty($result->payment_sdk_data->company_cat_id)){{$result->payment_sdk_data->company_cat_id}}@endif" placeholder="{{__('0_if_none')}}"/>
                                            </div>
                                            {!! formError($errors,'company_cat_id') !!}
                                        </div>
                                        <div class="form-group col-sm-6 {!! formError($errors,'order_type_id',true) !!}">
                                            <div class="controls">
                                                @if(!empty($result->payment_sdk_data->order_type_id))
                                                <div class="form-group">
                                                    {!! Form::label('order_type_id', __('order_type_id').':') !!}
                                                    <select required="" class="form-control" name="order_type_id"  id="order_type_id">
                                                        <option value="">إختر النوع</option>
                                                      <option @if(old('order_type_id',$result->payment_sdk_data->order_type_id) == 1) {{'selected'}} @endif value="1">فواتير</option>
                                                      <option  @if(old('order_type_id',$result->payment_sdk_data->order_type_id) == 5) {{'selected'}} @endif value="5">إعادة شحن</option>
                                                      <option  @if(old('order_type_id',$result->payment_sdk_data->order_type_id) == 6) {{'selected'}} @endif value="6">كارت</option>
                                                    </select>
                                                </div>
                                                @else
                                                    <div class="form-group">
                                                        ...
                                                        <select class="form-control" name="order_type_id"  id="order_type_id">
                                                            <option value="">إختر النوع</option>
                                                          <option @if(old('order_type_id') == 1) {{'selected'}} @endif value="1">فواتير</option>
                                                          <option  @if(old('order_type_id') == 5) {{'selected'}} @endif value="5">إعادة شحن</option>
                                                          <option  @if(old('order_type_id') == 6) {{'selected'}} @endif value="6">كارت</option>
                                                        </select>
                                                    </div>
                                                @endif
                                            </div>
                                            {!! formError($errors,'order_type_id') !!}
                                        </div>
                                    </div>
                                </div>
                            </div>
