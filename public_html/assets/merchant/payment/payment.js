...
$('body').on('submit','#services form',function(e){
    e.preventDefault();
    // console.log($(this).attr('action'),'test test');
        ...
        case 'prepaid':
            $.post(PostUrl,formData,function(PaymentData){
                ...
                if(PaymentData.status==true){
                    ...
                    console.log('success card',PaymentData);
                    $(element).html(CreateReceipt(PaymentData,printButton));

                } else {
                    console.log('error card',PaymentData)
                    $(element).html([
...
function CreateReceipt(data,printButton){
    console.log(data);
    var textData = data.data;
    var datetime = textData.dateTime?textData.dateTime.split(' '):null;
    var viewInfo = [];

    if(__('langCode') in textData['info']) {
        console.log(textData['info'][__('langCode')]);
        var varx = '';
        $.each(textData['info'][__('langCode')], function (i, val) {

            if (val !== 'undefined') {

              if (val['value'] === 'لا يوجد سجل للتاجر') {
                  varx = 'عملية خاطئة';
              }else if (val['value'] === 'No Record Found !') {
                  varx = 'error';
              }else if (val['value'] === 'Your amount is greater then of your balance') {
                  varx = 'error';
              }else if (val['value'] === 'لا يوجد لديك مبلغ كافى لﻹتمام العملية') {
                  varx = 'عملية خاطئة';
              }else if (val['value'] === 'Record Saved Successfully') {
                  varx = 'Successfuly';
              }else if (val['value'] === 'تم إتمام العملية بنجاح') {
                  varx = 'عملية ناجحة';
              }
              else if(val['value'] === 'Record is incorrect'){varx = 'error';}
              else if(val['value'] === 'مشكلة فى فئة الكارت'){varx = 'error';}
              else {
                  varx = 'عملية ناجحة';
              }
              viewInfo.push($('<tr>').append($('<td>', {text: val['key']+' :- '+val['value']})));
            }
        });
    }
    if(data.balance){
    ...
    return [$('<h2>',{text:data.msg}),
        $('<div>',{class:'col-xs-5 col-center',style:'text-align:center;font-family:"Lucida Grande","Neo Sans Arabic";',id:'bill'}).append([
            $('<div>',{style:'text-align:center'}).append($('<img>',{src:'https://e-yamamah.com/public/assets/merchant/image/logo.jpeg'})),
            $('<div>',{style:'text-align:center'}).append([
                ...
                $('<table>',{class:'table table-condensed'}).append([
                    ...
                    // $('<tr>').append([$('<td>',{text:__('Date')}),$('<td>',{text:d.getFullYear()+'-'+d.getDate().toString().padStart(2, '0')+'-'+d.getDate().toString().padStart(2, '0')})]),
                    // $('<tr>').append([$('<td>',{text:__('Time')}),$('<td>',{text:d.toLocaleTimeString().toLowerCase()})]),
                ...
                $('<div>',{class:'dividerB',html:$('<b>',{text:varx})}),
                ...
                //     $('<tr>').append([$('<td>',{text:'YAMAMAH'})]),
                //     $('<tr>').append([$('<td>',{text:'https://e-yamamah.com'})]),
                //     $('<tr>').append([$('<td>',{text:'Tel: +2 01212181321'})]),
