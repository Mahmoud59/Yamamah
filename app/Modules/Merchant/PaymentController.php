...
class PaymentController extends MerchantController
{
    ...
    public function index(Request $request){
        ...
        $this->viewData['services'] = PaymentServices::viewData(app()->getLocale(),[])->get();
    ...
    public function service($id,$specificType=false){
        ...
        if(!$service || ($specificType == 'payment')){
            ...
            if(count($service->payment_service_api_parameters)){
                ...
                $type = 'prepaid';
                $total = $this->getTotalAmount($id,0);
    ...
    //Start Code
    // Bee.php بدون الحاجة للرجوع الي ملف json  أضيفت حديثا لإظهار ناتج ال
...
// Start Original Code
        ...
        $service = PaymentServices::find($inputs['service_id']);

        if($service->payment_sdk_data != NULL)
        {
            foreach($service->payment_sdk_data as $key => $value) $inputs['parameters'][$key] = $value;
        }
...
//=============================================
//Start new code for testing momken api

   // $client = new Client();
    // Working api "http://api.alquran.cloud/v1/edition?format=audio&language=fr&type=versebyverse"
    //$res = $client->request('GET', 'https://smart-recharge.com/SmartWebService/SmartRecharge.svc/SaveOrder');
    //$res->getStatusCode();
    //$body = $res->getBody()->getContents();

   // return self::outPut(true,'Data sent successfully',20008,$data);

//=============================================
//End new code for testing momken api
