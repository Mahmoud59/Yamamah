...
class WalletController extends SystemController{
    ...
    public function transferMoneySupervisor(){
        ...
        $this->viewData['sendToStaff']  = Staff::with('paymentWallet')->find(Auth::user()->managed_staff);
    ...
    public function transferMoneyStaff(){
        ...
        $this->viewData['sendToStaff']  = Staff::with('paymentWallet')->find(Auth::user()->merchant);
    ...
    public function transferMoneyMainWallets(Request $request){
       ...
       $this->viewData['walletOUT'] = MainWallets::where('transfer_out','yes')->get(['id','name']);
        ...
        // $this->viewData['walletIN']  = MainWallets::with('wallet')->where('transfer_in','yes')->get(['id','name']);
        // $this->viewData['walletOUT'] = MainWallets::with('wallet')->where('transfer_out','yes')->get(['id','name']);
    ...
    public function transferMoneyMainWalletsPost(transferMoneyMainWalletsFormRequest $request){
        ...
        if($transactionStatus == 'pending'){
            ...
            $notifyStaff = Staff::join('permission_groups','permission_groups.id','=','staff.permission_group_id')
                ...
                ->where('permissions.route_name','system.wallet.transfer-money-main-wallets.post')
                ->get();
        ...
        if($request->isDataTable){
            ...
            return Datatables::eloquent($eloquentData)
                ...
                ->addColumn('walletowner_id', function($data){
                    if($data->walletowner_type == 'App\Models\MainWallets')
                    {
    ...
    public function transactions(Request $request){
        ...
        if($request->isDataTable){
            ...
            return Datatables::eloquent($eloquentData)
                ...
                ->addColumn('from_id', function($data){
                    $merchant = MerchantStaff::find($data->from_id);
                    return $merchant->firstname.' '.$merchant->lastname;
                })
                ->addColumn('to_id', function($data){
                    $merchant = MerchantStaff::find($data->to_id);
                    return $merchant->firstname.' '.$merchant->lastname;
                })
                ->addColumn('status', function($data){
                    return __($data->status);
