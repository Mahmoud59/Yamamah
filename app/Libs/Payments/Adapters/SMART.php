...
trait SMARTAdditionalFunction{
    ...
    private static function handleResponse($action, $response, $serviceData,$paymentTransaction = null){
        ...
        if (!$response) {
            return self::outPut(false, 'Service not available555', 20001);
        }
        $return = [];
...
//                setError(collect($serviceList)->toArray(),null,null);
    ...
    private static function generateXMLRequest($action, array $data, $locale){
        ...
        $xml->addChild('login', "اليمامة ");
        $xml->addChild('password', "201531");
        $xml->addChild('locale', $locale);
        $xml->addChild('ActionTypeID', 0);
 ...
 * @package App\Libs\Payments\Adapters
 */
class SMART implements \App\Libs\Payments\PaymentInterface{

    use SMARTAdditionalFunction;
    private static $response_obj = [];
    /**
     * @var int
     */
    ...
    private static function handleResponse($action, $response, $serviceData,$paymentTransaction = null){
        ...
        switch ($return['status']){
            ...
            case '20009':
                $serviceList = self::serviceList(1);
                Setting::where('name','payment_bee_service_version')
                    ->update(['value'=> (int)$serviceList->data->serviceVersion]);
                ...
                if (empty($text)) {
                    ...
                    if (!isset($newInfoData['ar'])) {
                        ...
                        foreach ($infoAllData as $key => $value) {
                            ...
                            if (gettype(json_decode($response->document->response)->result) == 'string') {
                            $BeeText[] = [
                                        'key'   => __('response'),
                                        'value' => __(json_decode($response->document->response)->result)
                                    ];
                                    if (!empty(json_decode($response->document->response)->phoneno)) {
                                    $BeeText[] = [
                                                'key'   => __('phoneno'),
                                                'value' => __(json_decode($response->document->response)->phoneno)
                                            ];
                                    }
                                    if (!empty(json_decode($response->document->response)->transactionId)) {
                                    $BeeText[] = [
                                                'key'   => __('transactionId'),
                                                'value' => __(json_decode($response->document->response)->transactionId)
                                            ];
                                    }

                            }
                            elseif (!empty(json_decode($response->document->response)->result) && gettype(json_decode($response->document->response)->result) == 'array')
                            {
                                if (!empty(json_decode($response->document->response,true)['result'][0])) {
                                    foreach (json_decode($response->document->response,true)['result'][0] as $key => $value) {
                                        if ($key == 'CardNo' || $key == 'CardSerial') {
                                            $BeeText[] = [
                                                'key'   => __($key),
                                                'value' => __($value)
                                            ];
                                    ...
                                    foreach (json_decode($response->document->response)->result as $key => $value) {
                                        $BeeText[] = [
                                            'key'   => __($key),
                                            'value' => __($value)
                                        ];
    ...
    $service = PaymentServices::find($serviceID);
    if (!empty($service->payment_sdk_data)) {

      $sdk_data =   $service->payment_sdk_data;


    }
    else{
        return self::outPut(false,'we cant find this service Itachi ',103);
    }
       // $generateXMLRequest = static::generateXMLRequest($action,$data,$locale);
        $BeeUrl = 'https://smart-recharge.com/SmartWebService/SmartRecharge.svc/SaveOrder';
        // Rrequests Hooks

            if ($sdk_data->order_type_id == "1" || $sdk_data->order_type_id == "5") {
                foreach ($data['requestMap'] as $key => $value) {
            ...
            elseif ($sdk_data->order_type_id == "6") {

                 $xdata['CatID']             = $sdk_data->company_cat_id;
            }

            $xdata['UserName']          = trim("اليمامة");
            $xdata['Password']          = trim("201531");
            $xdata['ClientName']        = trim("اليمامة");

            $xdata['CompanyID']         = $sdk_data->company_id;
            $xdata['OrderTypeID']       = $sdk_data->order_type_id ;
            $xdata['ActionTypeID']      = trim("0");
            $json = json_encode($xdata, JSON_UNESCAPED_UNICODE);
        try{

        $response = Requests::post($BeeUrl,['Content-Type' => 'application/json; charset=utf-8'],$json,['timeout'=>50,'connect_timeout'=>50]);
...
if (!empty($xdata['phoneno']) || !empty($data['transactionId'])) {
    $xdatae = json_decode($response->body,true);
    if (!empty($xdata['phoneno'])) {
        $xdatae['phoneno'] = $xdata['phoneno'];
    }
    if (!empty($data['transactionId'])) {
       $xdatae['transactionId'] = $data['transactionId'];
    }

    $xdatae = json_encode($xdatae);
}
else{
    $xdatae = $response->body;
}
$movies->document->response = $xdatae;
    ...
    public static function inquiry($requestMap = []){
        ...
        if(!$getServiceData || !$getServiceData->payment_sdk || !$getServiceData->payment_service_provider){
            return self::outPut(false,'Service not available111',401);
        }
        ...
        if(!$paymentTransaction){
            return self::outPut(false,'Service not available222',402);
        }
    ...
    public static function payment($requestMap = [], $inquiryTransactionsID = false,$amount = false){
        ...
        if(!$inquiryTransactionsID && !$amount){
            return self::outPut(false,'Service not available333',20007);
        }
    ...
    public static function totalAmount($amount){
        ...
        if(!$getServiceData || !$getServiceData->payment_sdk || !$getServiceData->payment_service_provider){
            return self::outPut(false,'Service not available444',20014);
        }
